...
def create_llm_client(provider="openai"):
    if provider == "openai":
        ...
        api_key = os.getenv("AI_BUILDER_TOKEN") or os.getenv("AI_BUILDER_API_KEY")
        if not api_key:
            raise ValueError(
                "AI_BUILDER_TOKEN or AI_BUILDER_API_KEY not found in environment variables "
                "(Space API token; Cursor MCP mcp.json may use AI_BUILDER_TOKEN)"
            )
        raw_base = (
            os.getenv("AI_BUILDER_BASE_URL") or "https://space.ai-builders.com/backend/v1"
        ).strip().rstrip("/")
        if raw_base.endswith("/backend"):
        ...
        # OpenAI-compatible local server (default: Ollama http://127.0.0.1:11434/v1).
        base_url = (
...
def main():
    ...
    parser.add_argument(
        '--provider',
        choices=['openai', 'anthropic', 'gemini', 'local', 'deepseek', 'azure', 'siliconflow', 'aibuilder'],
        default='openai',
    ...
    if not args.model:
        ...
        elif args.provider == "aibuilder":
            args.model = os.getenv("AI_BUILDER_MODEL", "grok-4-fast")
        elif args.provider == "local":
            args.model = os.getenv("LOCAL_LLM_MODEL", "llama3.2")
